...
func TestToJSON_AllKinds(t *testing.T) {
	cases := []struct {
		...
		{"ToolCallFailed", event.KindToolCallFailed, event.ToolCallFailed{CallID: "c1", Error: "boom", ErrorType: event.ToolErrorPanic, Attempt: 1}, "error_type"},
		{"SideEffectRecorded", event.KindSideEffectRecorded, event.SideEffectRecorded{Name: "now"}, "name"},
		{"BudgetExceeded", event.KindBudgetExceeded, event.BudgetExceeded{Limit: event.LimitUSD, Cap: 0.5, Actual: 0.6, Where: event.WhereMidStream}, "limit"},
		...
		{"RunFailed", event.KindRunFailed, event.RunFailed{Error: "boom", ErrorType: event.RunErrorProvider}, "error_type"},
